...
bavaria <- st_read("../datasets/chapter_2/bavaria.shp")
# nicer colnames
colnames(bavaria) <- c(
...
bavaria$gdp_per_capita <- as.numeric(as.character(
  bavaria$gdp_per_capita
))
bavaria$mean_age <- as.numeric(as.character(
  bavaria$mean_age
))
bavaria$pop_density <- as.numeric(as.character(
  bavaria$pop_density
))
bavaria$unemployment_rate <- as.numeric(as.character(
  bavaria$unemployment_rate
))
bavaria$employment_rate <- as.numeric(as.character(
  bavaria$employment_rate
))
bavaria$household_income <- as.numeric(as.character(
  bavaria$household_income
))
bavaria$students <- as.numeric(as.character(
  bavaria$students
))
bavaria$pop_development <- as.numeric(
  as.character(bavaria$pop_development)
)
bavaria$type <- ifelse(bavaria$type == "Landkreis", "District", "City")
# load europe elevation raster
europe_raster <- raster("../datasets/chapter_2/elevation1x1_new.tif")
# load europe shapefiles
europe_shape <- read_sf("../datasets/chapter_2/ne_50m_admin_0_countries/ne_50m_admin_0_countries.shp")
...
gemeinden <- read_sf("../datasets/chapter_2/gmd_ex.shp")
rosenheim <- gemeinden[str_detect(gemeinden$BEZ_KRS, "Rosenheim"), ]

honey <- read_csv("../datasets/chapter_2/honeyproduction.csv")
...
```{r, eval=FALSE}
# for loading and wrangling our data
library(raster)
library(readr)
library(readxl)
library(sf)
...
```{r, fig.align="center",fig.cap="Unemployment rate in Bavaria and distribution of cities and districts", echo = FALSE, fig.width= 8, fig.height= 8, warning=FALSE, message=FALSE}
...
tm_shape(bavaria) + tm_polygons(col = "type", pal = c("white", "skyblue")) +
  tm_logo("../datasets/chapter_2/bavaria.png", height = 2) + tm_scale_bar(position = c("left", "bottom"), width = 0.15) +
  tm_compass(position = c("left", "top"), size = 2)
...
honey0912 <- rbind(
  ...
  matrix(NA, ncol = ncol(honey0912),
         nrow = 4 * length(unique(abbrev$state)[!unique(abbrev$state) %in% unique(honey0912$state)]),
         dimnames = list(
           c(seq_len(4 * length(unique(abbrev$state)[!unique(abbrev$state) %in% unique(honey0912$state)]))),
           colnames(honey0912)
         )
  )
)
honey0912$state <- as.character(honey0912$state)
honey0912[is.na(honey0912$state), ]$state <- sort(rep(unique(abbrev$state)[!unique(abbrev$state) %in% unique(honey0912$state)], 4))
...
```
```{r, eval = FALSE}
honey <- read_sf("../datasets/chapter_2/honey.shp")
...
tmap_animation(honey_animation, filename = "../datasets/chapter_2/honey.gif", delay = 50,
               width = 2400, height = 1200)
```
```{r, fig.align="center",fig.cap="Animated map", echo=FALSE}
...
```{r, warning=FALSE, fig.align="center", fig.cap="Faceted interactive map", eval = FALSE}
