...
Als erstes wird ein Teildatensatz erstellt, der alle Daten aus dem ausgewählten Jahr für die selektierte Variable enthält. Die von dem Benutzer ausgewählten Werte werden mit `input$inputId` abgerufen. In diesem Fall `input$year`, `input$variable` und `input$palette`. Diese haben immer die Klassen **character**, für das Jahr muss dieser Wert also in einen numerischen umgewandelt werden.
Die Funktion `colorNumeric` wird für die Erstellung der Farbpalette verwendet, ehe ein Titel für die Legende erstellt wird und die Karte generiert wird. Mehr zu der Erstellung der Karte und des Leaflet Pakets in dem entsprechenden Kapitel.  
```{r, eval = FALSE}
